...
p01 = ggplot(fava_do, 
       aes(y = FAVA, x = lat_start, color = start_mya %>% as.factor)) +
  geom_point() + 
  facet_grid(data ~ start_mya_bin)+
  geom_line(alpha=0.5) + 
  scale_color_viridis_d(option = "E", direction = -1, name = "Window\nstart\n(mya)")  + 
  theme_bw()+
  xlab("Southernmost bound of 5-degree paleo latitude bin")

p02 = ggplot(fava_do %>% filter(!is.na(mean_d18O_sw)), 
       aes(y = mean_d18O_sw, x = lat_start, color = start_mya %>% as.factor)) +
  geom_point() + 
  facet_grid( ~ start_mya_bin)+
  geom_line(alpha=0.5) + 
  scale_color_viridis_d(option = "E", direction = -1, name = "Window\nstart\n(mya)")  + 
  theme_bw() + 
  ylab("Mean d18O") +
...
p03 = ggplot(fava_do %>% filter(!is.na(mean_d18O_sw)) %>% arrange(lat_start), 
       aes(x = mean_d18O_sw, y = FAVA, fill = lat_start)) +
  geom_path(aes(group = start_mya %>% factor, color = start_mya %>% factor), alpha = 0.5) +
  ...
  scale_fill_gradientn(colors = RColorBrewer::brewer.pal(11, "Spectral"), 
                       limits = c(-66,66), 
                       name = "Paleo\nlatitude") +
  xlab("Mean d18O (in a 5-degree paleo latitude bin & a 5-My window)") + 
  scale_color_viridis_d(option = "E", direction = -1, name = "Window\nstart\n(mya)", guide = "none")

panels_fava = wrap_plots(p01, p02, p03, guides = "collect", ncol = 1, heights = c(3,1,3)) + plot_annotation(tag_levels = "A")

# ggsave("../Figures/lat_fava_grad_temperature.png", panels_fava, width = 25, height = 25, scale = 0.37)
